...
for cluster in clusters:
    print_nodelist = True
    nodes = run_command_to_list("sinfo -M {0} -t mix,alloc,idle -N -o %N -h".format(cluster))[1:]
    for node in nodes:
        if not node == '':
            if print_nodelist:
                nodelist[cluster] = []
...
for cluster in nodelist.keys():
    for node in nodelist[cluster]:
        ...
        slurm_jobs = run_command_to_list("squeue -h -M {0} -w {1} -o %A".format(cluster, node))[1:]
        slurm_users = []
        ...
        for line in node_processes_raw:
            ...
            try:
                ...
                with open("/zfs1/crc/logs/shinigami/{0}.log".format(node), 'a') as log:
                    log.write("--> {0} <--\n".format(datetime.now()))
        ...
        if len(admin_log) != 0:
            with open("/zfs1/crc/logs/shinigami/{0}-admin.log".format(node), 'a') as log:
                log.write("--> {0} <--\n".format(datetime.now()))
